USE [TestCrifis2]
GO
/****** Object:  StoredProcedure [dbo].[sp_DataExchange_DataLoading]    Script Date: 05/02/2026 12:59:06 PM ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
ALTER PROCEDURE [dbo].[sp_DataExchange_DataLoading]
...
BEGIN
	...
	SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;
		...
		DECLARE @UID nvarchar(25),@UpdateDate DATE,@Subject NVARCHAR(100),@SubjectNativeName NVARCHAR(100),
		...
		@DTRanking int = null
		Declare @DTSourceID varchar(2), @DTIntelligenceID varchar(2)
    ...
    Comment, CapitalCurrency, DateReported, DateUpdated, Source, Intelligence)
		...
		INSERT INTO #Financials (FinancialDate, MONTHSNO, Denomination, [YEAR], [Type], [Audit], [Name], StatementType, FinancialValue, FinancialCurrency)
		SELECT
		JSON_VALUE(Financial.value, '$.FINANCIALDATE') AS FinancialDate,
		--CONVERT(date, JSON_VALUE(Financial.[value], '$.FINANCIALDATE'), 103) AS FinancialDate,
		JSON_VALUE(Financial.value, '$.MONTHSNO') AS MONTHSNO,
		JSON_VALUE(Financial.value, '$.Denomination') AS Denomination,
		JSON_VALUE(Financial.value, '$.YEAR') AS [YEAR],
		JSON_VALUE(Financial.value, '$.TYPE') AS [Type],
		JSON_VALUE(Financial.value, '$.AUDIT') AS [Audit],
		JSON_VALUE(Financial.value, '$.NAME') AS [Name],
		JSON_VALUE(Financial.value, '$.STATEMENTTYPE') AS StatementType,
		CAST(JSON_VALUE(Financial.value, '$.FINANCIALVALUE') AS DECIMAL(18,3)) AS FinancialValue,
		JSON_VALUE(Financial.value, '$.FINANCIALCURRENCY') AS FinancialCurrency
		FROM OPENJSON(@Json, '$.FINANCIALS.FINANCIAL') AS Financial
		...
		ParsedStatements AS (
			SELECT 
				...
				--JSON_VALUE(s.[value], '$.FINANCIALDATE') AS FINANCIALDATE,
				CONVERT(date, JSON_VALUE(s.[value], '$.FINANCIALDATE'), 103) AS FinancialDate,		--Changed on 26/01/2026 by Vivek
				JSON_VALUE(s.[value], '$.MONTHSNO') AS MONTHSNO,
				JSON_VALUE(s.[value], '$.Denomination') AS Denomination,
				JSON_VALUE(s.[value], '$.YEAR') AS [YEAR],
				JSON_VALUE(s.[value], '$.TYPE') AS [Type],
				JSON_VALUE(s.[value], '$.AUDIT') AS [Audit],
				JSON_VALUE(s.[value], '$.STATEMENTTYPE') AS STATEMENTTYPE,
				JSON_VALUE(s.[value], '$.FINANCIALCURRENCY') AS FINANCIALCURRENCY,
				JSON_QUERY(s.[value], '$.Analyses') AS Analyses
			FROM Statement s
		),
    ...
    BEGIN
        ...
        DECLARE reg_cursor CURSOR FOR
            ...
            WHERE REGISTERTYPEID = 4024813 
              ...
              AND ISNULL(REGISTERNUMBER, '') <> '';

        OPEN reg_cursor;
        FETCH NEXT FROM reg_cursor INTO @RegisterNumber;

        WHILE @@FETCH_STATUS = 0
        BEGIN
            PRINT '  Checking RegisterNumber: ' + ISNULL(@RegisterNumber, 'NULL');

            SET @UID = (
                SELECT TOP 1 a.[UID]
                FROM tblatoms a WITH (NOLOCK)
                JOIN tblcompanies c WITH (NOLOCK) ON c.IDATOM = a.IDATOM
                JOIN tblCompanyIDs i WITH (NOLOCK) 
                    ON i.IDATOM = a.IDATOM 
            ...
            ELSE
            BEGIN
                PRINT 'No match found for this register number.';
            END
...
PRINT '  idatom = ' + ISNULL(CAST(@idatom AS NVARCHAR(50)), 'NULL');
		...
		BEGIN
			...
			END
			DECLARE @NameId INT,@name NVARCHAR(100),@IsNative int,@IsHistory int, @TdateStart date, @TdateEnd date;
			DECLARE cur_FormerData_Transfer CURSOR FOR

			SELECT id,[Name],IsNative,IsHistory, STARTDATE, ENDDATE
			FROM #TradingNames WITH(NOLOCK)
		
				OPEN cur_FormerData_Transfer
				FETCH NEXT FROM cur_FormerData_Transfer INTO @NameId,@name,@IsNative,@IsHistory,@TdateStart,@TdateEnd
				WHILE @@FETCH_STATUS = 0
				BEGIN

			IF((SELECT count(tblcompanies_names.Id) from tblcompanies_names where idatom = @idatom and [Name]=@name) >0)
			BEGIN
			
				set @DTSourceID = isnull((SELECT TOP 1 s.[Rank] from tblcompanies_names c join tblRecordsSources s on c.SourceID=s.ID where idatom = @idatom and c.[Name]=@name ),12)
				set @DTIntelligenceID = isnull((SELECT TOP 1 s.[Rank] from tblcompanies_names c join tblRecordsIntelligence s on c.IntelligenceID=s.ID  where idatom = @idatom  and c.[Name]=@name),7)
				set @DTRanking = (SELECT Ranking from tblRecordsSIranking where [Source]=@DTSourceID and [Intelligence]=@DTIntelligenceID)
				...
				--IF((@DTSourceID is null or @DTIntelligenceID is null) ) --or @DTRanking >= @RankingID
					...
					BEGIN		
						...
						UPDATE tblcompanies_names set 
							DateUpdated = GETDATE(),SourceId=@SourceID,IntelligenceId=@IntelligenceID,
							isHistory = @IsHistory, NameType = @IsNative, startDate= @TdateStart, EndDate= @TdateStart
			...
			BEGIN
					...
					SELECT DISTINCT @idatom,IsNative,[Name],@IntelligenceID as IntelligenceID,@SourceID as SourceID,IsHistory,GETDATE(),GETDATE(), STARTDATE, ENDDATE
					from #TradingNames
					where ID=@NameId					
	...
	--END
		...
		BEGIN
			...
			SELECT Year
			FROM #NUMBEROFEMPLOYEES WITH(NOLOCK)
		
				OPEN cur_FormerData_Transfer
				FETCH NEXT FROM cur_FormerData_Transfer INTO @Year
				WHILE @@FETCH_STATUS = 0
				BEGIN

			IF((SELECT COUNT(id) from tblCompanies_Employees where idatom = @idatom)>0 and (select count(*) from #NUMBEROFEMPLOYEES)>0)
			BEGIN

				set @DTSourceID = ISNULL((SELECT s.[Rank] from tblCompanies_Employees c join tblRecordsSources s on c.SourceID=s.ID  where idatom = @idatom and c.Year=@Year),12)
				set @DTIntelligenceID = ISNULL((SELECT s.[Rank] from tblCompanies_Employees c join tblRecordsIntelligence s on c.IntelligenceID=s.ID  where idatom = @idatom and c.Year=@Year),7)
				set @DTRanking = (SELECT Ranking from tblRecordsSIranking where [Source]=@DTSourceID and [Intelligence]=@DTIntelligenceID)

				IF((@DTSourceID is null or @DTIntelligenceID is null) ) or @DTRanking >= @RankingID
					BEGIN	

						IF((Select TotalNumberFrom from tblCompanies_Employees where IDATOM=@idatom and Year=@Year)<(SELECT NumberFrom from #NUMBEROFEMPLOYEES where Year=@Year))
					
						BEGIN
						
							UPDATE tblCompanies_Employees set UpdatedDate = getdate(),SourceID = @SourceID,IntelligenceID = @IntelligenceID	
							where idatom = @idatom					
	
							UPDATE e set TotalNumberFrom = NumberFrom
...
END
		...
		BEGIN
			...
			--	INSERT INTO TestCrifis2.dbo.tblCompanies_OriginalText(IDATOM, Managers, SourceID,IntelligenceID)
